    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        const dbInput = {
          ...
          meetingTitle: input.meetingTitle ?? "Unknown Meeting Title",
          meetingInfo: input.meetingInfo,
          startTime: input.startTime ?? new Date(),
          endTime: input.endTime ?? new Date(),
          heartbeatInterval: input.heartbeatInterval ?? 1000,
          automaticLeave: input.automaticLeave ?? {
            waitingRoomTimeout: 10000,
            noOneJoinedTimeout: 10000,
            everyoneLeftTimeout: 10000,
          },
        };
