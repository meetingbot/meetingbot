  ...
  listApiKeys: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      return await ctx.db
        .select()
        .from(apiKeys)
        .where(eq(apiKeys.userId, ctx.auth.user.id));
    }),
  ...
  revokeApiKey: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const apiKey = await ctx.db
        ...
        .from(apiKeys)
        .where(
          and(eq(apiKeys.id, input.id), eq(apiKeys.userId, ctx.auth.user.id)),
        );

      if (!apiKey[0] || apiKey[0].userId !== ctx.auth.user.id) {
      ...
      const logs = await ctx.db
        ...
        .where(
          and(
            ...
            eq(apiRequestLogs.userId, ctx.auth.user.id),
          ),
        )
  ...
  getApiKeyCount: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      const countResult = await ctx.db
        ...
        .where(
          and(
            eq(apiKeys.userId, ctx.auth.user.id),
