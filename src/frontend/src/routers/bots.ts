import { z } from "zod";
import { router, procedure, protectedProcedure } from "../server/trpc";
import {
  bots,
  events,
  insertBotSchema,
...
} from "~/server/services/botDeployment";
...
export const botsRouter = router({
  getBots: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      return await ctx.db
        .select()
        .from(bots)
        .where(eq(bots.userId, ctx.auth.user.id))
  ...
  getBot: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!result[0] || result[0].userId !== ctx.auth.user.id) {
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        const dbInput = {
          ...
          heartbeatInterval: input.heartbeatInterval ?? 10000, // 10 seconds
          automaticLeave: input.automaticLeave ?? {
            waitingRoomTimeout: 1000 * 60 * 10, // 10 minutes
            noOneJoinedTimeout: 1000 * 60 * 10, // 10 minutes
            everyoneLeftTimeout: 1000 * 60 * 10, // 10 minutes
          },
        };
  ...
  updateBot: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!bot[0] || bot[0].userId !== ctx.auth.user.id) {
        throw new Error("Bot not found");
      }
  ...
  getActiveBotCount: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      const result = await ctx.db
        ...
        .where(
          and(
            eq(bots.userId, ctx.auth.user.id),
