...
import { type Env, type BotConfig } from "../../../backend/src/db/schema";

// Load environment variables
dotenv.config();

const env = process.env as unknown as Env;
const DATA = JSON.parse(env.BOT_DATA || "{}") as unknown as BotConfig;
const { meetingInfo, botDisplayName, heartbeatInterval, automaticLeave } = DATA;
const { meetingId, organizerId, tenantId } = meetingInfo;
const waitingRoomTimeout = automaticLeave?.waitingRoomTimeout ?? 0; // 0 disables timeout
